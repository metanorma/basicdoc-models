...
The atlas (GitHub Pages, `metanorma.github.io/basicdoc-models`) is generated by `site/generate.rb` from `views/*.lml` metadata + `images/`; templates live in `site/templates/`. It is a derived view — never hand-edit `_site/`, and never add site content that is not backed by LML. Deployed by `.github/workflows/pages.yml` on push to `main`.
